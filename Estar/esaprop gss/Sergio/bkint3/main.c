...
int x=0;
int y=0;
long int tel_pack_number;
long int saved_pack_number;
...
//  sprintf(sec_char,"%d",sec); 
  ...
  char volt5_curr_c[7];	
//  sprintf(volt5_curr_c,"%d",volt5_curr);

  char volt3_curr_c[7];
//  sprintf(volt3_curr_c,"%d",volt3_curr);

  char avio_bat1_volt_c[7]; 
//  sprintf(avio_bat1_volt_c,"%d",avio_bat1_volt);

  char avio_bat2_volt_c[7];
//  sprintf(avio_bat2_volt_c,"%d",avio_bat2_volt);
 
  char avio_bat1_curr_c[7];
//  sprintf(avio_bat1_curr_c,"%d",avio_bat1_curr);

  char avio_bat2_curr_c[7];
//  sprintf(avio_bat2_curr_c,"%d",avio_bat2_curr);
 
  char avio_bat1_temp_c[7];
//  sprintf(avio_bat1_temp_c,"%d",avio_bat1_temp);
 
  char avio_bat2_temp_c[7];
//  sprintf(avio_bat2_temp_c,"%d",avio_bat2_temp);

  char avio_bat1_chcurr_c[7];
//  sprintf(avio_bat1_chcurr_c,"%d",avio_bat1_chcurr);

  char avio_bat2_chcurr_c[7];
//  sprintf(avio_bat2_chcurr_c,"%d",avio_bat2_chcurr);

  char ps_bat1_volt_c[7];
//  sprintf(ps_bat1_volt_c,"%d",ps_bat1_volt);
 
  char ps_bat2_volt_c[7];
//  sprintf(ps_bat2_volt_c,"%d",ps_bat2_volt);
 
  char ps_bat1_curr_c[7];
//  sprintf(ps_bat1_curr_c,"%d",ps_bat1_curr);
 
  char ps_bat2_curr_c[7];
//  sprintf(ps_bat2_curr_c,"%d",ps_bat2_curr);
 
  char ps_bat1_temp_c[7]; 
//  sprintf(ps_bat1_temp_c,"%d",ps_bat1_temp);

  char ps_bat2_temp_c[7];
//  sprintf(ps_bat2_temp_c,"%d",ps_bat2_temp);
 
  char ps_bat1_chcurr_c[7];
//  sprintf(ps_bat1_chcurr_c,"%d",ps_bat1_chcurr);
 
  char ps_bat2_chcurr_c[7];
//  sprintf(ps_bat2_chcurr_c,"%d",ps_bat2_chcurr);
  
  char step_up_curr_c[7];
//  sprintf(step_up_curr_c,"%d",step_up_curr);

  char step_up_volt_c[7];
//  sprintf(step_up_volt_c,"%d",step_up_volt);

  char dac_command_c[7];
//  sprintf(dac_command_c,"%d",dac_command);

  char extra1_temp_c[7];
//  sprintf(extra1_temp_c,"%d",extra1_temp);

  char extra2_temp_c[7];
//  sprintf(extra2_temp_c,"%d",extra2_temp);
  
  char extra3_temp_c[7];
//  sprintf(extra3_temp_c,"%d",extra3_temp);

  char extra4_temp_c[7];
//  sprintf(extra4_temp_c,"%d",extra4_temp);

  char extra5_temp_c[7];
//  sprintf(extra5_temp_c,"%d",extra5_temp);

  char extra6_temp_c[7];
//  sprintf(extra6_temp_c,"%d",extra6_temp);

  char extra7_temp_c[7];
//  sprintf(extra7_temp_c,"%d",extra7_temp);

  char magnet_x_c[7];
//  sprintf(magnet_x_c,"%d",magnet_x);

  char magnet_y_c[7];
//  sprintf(magnet_y_c,"%d",magnet_y);

  char magnet_z_c[7];
//  sprintf(magnet_z_c,"%d",magnet_z);
/*
  char accel_x=dati_finale[31];
  char accel_y=dati_finale[32];
  char accel_z=dati_finale[33];
  char force1=dati_finale[34];
  char force2=dati_finale[35];
  char force3=dati_finale[36];
*/
...
//Status Frame
  ...
  label43 = gtk_label_new("Reboot Number");
	...
	while(1)
	{
		...
		if (packet_display==0)
		{
			...
			//Spacchetto config_status in tel_pack
			 tel_pack_number=(long int)(pack_in_HL[9]*255*255*255+pack_in_HL[10]*255*255+pack_in_HL[11]*255+pack_in_HL[12]);
			//Spacchetto config_status in saved pack
			 saved_pack_number=(long int)(pack_in_HL[13]*255*255*255+pack_in_HL[14]*255*255+pack_in_HL[15]*255+pack_in_HL[16]);
			
			for(t=0,p=0;p<36;t=t+2,p++)
			{
				dati_finale[p]=pack_in_HL[20+t]*256+pack_in_HL[20+t+1];
		...
		else if (packet_display==1)
		{
			...
			//Spacchetto config_status in tel_pack
			 tel_pack_number=(long int)(pack_in_TNC[24]*255*255*255+pack_in_TNC[25]*255*255+pack_in_TNC[26]*255+pack_in_TNC[27]);
			//Spacchetto config_status in saved pack
			 saved_pack_number=(long int)(pack_in_TNC[28]*255*255*255+pack_in_TNC[29]*255*255+pack_in_TNC[30]*255+pack_in_TNC[31]);

			for(t=0,p=0;p<36;t=t+2,p++)
			{
				dati_finale[p]=pack_in_TNC[35+t]*256+pack_in_TNC[35+t+1];
...
printf("		5V bus current: %d				3V bus current: %d\n", volt5_curr,volt3_curr);
