...
int x=0;
int y=0;
char tel_pack_number[4];
char saved_pack_number[4];
char opmode;
char last_comm;
char reboot_num;
...
else if (pid >0)
{
	while(1)
	{
		...
		if (packet_display==0)
		{
			...
			x=0;
			y=0;
			opmode=pack_in_TNC[17];
			last_comm=pack_in_TNC[18];
			reboot_num=pack_in_TNC[19];
			//Spacchetto config_status in tel_pack
			for(x=0;x<4;x++)
			{
			  tel_pack_number[x]=pack_in_TNC[9+x];
			}
	
			//Spacchetto config_status in saved pack
			for(y=0;y<4;y++)
			{
			  tel_pack_number[y]=pack_in_TNC[13+y];
			}
		...
		else if (packet_display==1)
		{
			...
			for(x=0;x<4;x++)
			{
			  tel_pack_number[x]=pack_in_TNC[24+x];
			}
			
			//Spacchetto config_status in saved pack
			for(y=0;y<4;y++)
			{
			  tel_pack_number[y]=pack_in_TNC[28+y];
			}
			...
			//Definizione delle stringhe da stampare nelle entries
			  ...
			  sprintf(year_char,"%c",year); 
  
			  sprintf(month_char,"%c",month); 

			  sprintf(day_char,"%c",day); 

			  sprintf(hour_char,"%c",hour); 

			  sprintf(min_char,"%c",min); 

			  sprintf(sec_char,"%c",sec); 
