...
//Variabili globali da proiettare a schermo
float volt5_curr_f=0;	
float volt3_curr_f=0;
float avio_bat1_volt_f=0; 
float avio_bat2_volt_f=0; 
float avio_bat1_curr_f=0;
float avio_bat2_curr_f=0; 
float avio_bat1_temp_f=0; 
float avio_bat2_temp_f=0;
float avio_bat1_chcurr_f=0;
float avio_bat2_chcurr_f=0;
float ps_bat1_volt_f=0; 
float ps_bat2_volt_f=0; 
float ps_bat1_curr_f=0; 
float ps_bat2_curr_f=0; 
float ps_bat1_temp_f=0; 
float ps_bat2_temp_f=0; 
float ps_bat1_chcurr_f=0; 
float ps_bat2_chcurr_f=0;  
float step_up_curr_f=0;
float step_up_volt_f=0;
float extra1_temp_f=0;
float extra2_temp_f=0;
float extra3_temp_f=0;
float extra4_temp_f=0;
float extra5_temp_f=0;
float extra6_temp_f=0;
float extra7_temp_f=0;
float magnet_x_f=0;
float magnet_y_f=0;
float magnet_z_f=0;
float accel_x_f=0;
...
//Definizione pacchetti
	...
	//Tutti erano GtkEntryBuffer con 20 chars
	char year_char[20];
	char month_char[20];
	char day_char[20];
	char hour_char[20];
	...
	//Definizione delle struct dei pacchetti ricevuti
...
//Acquisizione comandi da passare al CTP
...
//dichiarazione Widget GTK per comandi
	...
	//g_timeout_add(1000,function1,NULL);
	gtk_main();
...
int main(int argc, char *argv[]){
	...
	//Definizioni delle stringhe da stampare nelle entries
	...
	//Inizializzazioni
	serial_number= serial_init();	//apertura seriale 1
	if (serial_number<0){
	...
	serial_number1= serial_init2();			//apertura seriale 2
	if (serial_number1<0){
	...
	for (i=0;i<18;i++) //header RF command
		pack_out_TNC[i] = command_ax25_start[i];

	//----------------------------------
	printf("Il comando con solo header è : %c %c %c", pack_out_HL[0],pack_out_HL[1],pack_out_HL[2] );

	//creazione thread acquisizione dati
	iret1 = pthread_create(&thread1,NULL,function1,(void *)NULL);
	...
	//labelTest = gtk_label_new("2019");

	telemetry35= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry35),(const gchar*)year_char);
	//gtk_entry_set_text(GTK_ENTRY(telemetry35),year_char);
	gtk_widget_set_sensitive (telemetry35, FALSE);
	gtk_box_pack_start(GTK_BOX(Hbox35), label35, TRUE, TRUE, 0);//___
	gtk_box_pack_start(GTK_BOX(Hbox35), telemetry35, TRUE, TRUE, 0);//___
    //gtk_widget_show_all(telemetry35); //new add

	Hbox36 = gtk_hbox_new(TRUE, 1);
	gtk_container_add(GTK_CONTAINER(vbox_time), Hbox36);
	label36 = gtk_label_new("Month");
	telemetry36= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry36),(const gchar*)month_char);
	...
	telemetry37= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry37),(const gchar*)day_char);
	...
	telemetry38= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry38),(const gchar*)hour_char);
	...
	telemetry39= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry39),(const gchar*)min_char);
	...
	telemetry40= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry40),(const gchar*)sec_char);
	...
	telemetry41= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry41),(const gchar*)opmode_c);// da spacchettare?
	...
	telemetry42= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry42),(const gchar*)last_comm_c);// da spacchettare?
	...
	telemetry44= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry44),(const gchar*)reboot_num_c);// da spacchettare?
	...
	telemetry45= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry45),(const gchar*)tel_pack_number_c);// da spacchettare?
	...
	telemetry43= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry43),(const gchar*)saved_pack_number_c);// da spacchettare?
  ...
  //gtk_container_add(GTK_CONTAINER(frame2), vbox2);
	...
	telemetry4=gtk_entry_new_with_buffer(avio_bat1_volt_b);
	gtk_entry_buffer_set_text(avio_bat1_volt_b,avio_bat1_volt_c,-1); // Da sistemare?
	...
	telemetry5= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry5),(const gchar*)avio_bat1_curr_c);
	...
	telemetry6= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry6),(const gchar*)avio_bat1_temp);
	...
	telemetry7= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry7),(const gchar*)avio_bat1_chcurr_c);
	...
	telemetry8= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry8),(const gchar*)avio_bat2_volt_c);
	...
	telemetry9= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry9),(const gchar*)avio_bat2_curr_c);
	...
	telemetry10= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry10),(const gchar*)avio_bat2_temp_c);
	...
	telemetry11= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry11),(const gchar*)avio_bat2_chcurr_c);
	...
	telemetry12= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry12),(const gchar*)ps_bat1_volt_c);
	...
	telemetry13= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry13),(const gchar*)ps_bat1_curr_c);
	...
	telemetry14= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry14),(const gchar*)ps_bat1_temp_c);
	...
	telemetry15= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry15),(const gchar*)ps_bat1_chcurr_c);
	...
	telemetry16= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry16),(const gchar*)ps_bat2_volt_c);
	...
	telemetry17= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry17),(const gchar*)ps_bat2_curr_c);
	...
	telemetry18= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry18),(const gchar*)ps_bat2_temp_c);
	...
	telemetry19= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry19),(const gchar*)ps_bat2_chcurr_c);
	...
	telemetry20= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry20),(const gchar*)volt5_curr_c);
	...
	telemetry21= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry21),(const gchar*)volt3_curr_c);
	...
	telemetry22= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry22),(const gchar*)step_up_volt_c);
	...
	telemetry23= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry23),(const gchar*)step_up_curr_c);
	...
	telemetry28= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry28),(const gchar*)extra1_temp_c);
	...
	telemetry29= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry29),(const gchar*)extra2_temp_c);
	...
	telemetry30= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry30),(const gchar*)extra3_temp_c);
	...
	telemetry31= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry31),(const gchar*)extra4_temp_c);
	...
	telemetry32= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry32),(const gchar*)extra5_temp_c);
	...
	telemetry33= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry33),(const gchar*)extra6_temp_c);
	...
	telemetry34= gtk_label_new("");
	gtk_label_set_text(GTK_LABEL(telemetry34),(const gchar*)extra7_temp_c);
	...
	while(1){
		...
		for(ii=0;ii<500;ii++){
			...
			if (HL_validation==1){
				...
				printf ("pack_in_HL[%d]=%x\t",jj,pack_in_HL[jj]);
			}	
			printf("\n");
			f_HL=fopen("log_HL.txt","a");
				//printf("error in the opening log_HL.txt\n");
			fprintf(f_HL,"%s\n",pack_in_HL);
			fclose(f_HL);
		}
		...
		for(ii=0;ii<500;ii++){
			...
			if (TNC_validation==15)//Al termine del controllo se il flag==15 (ossia tutti gli elementi dell'header corrispondono)
			{
				for(jj=0;jj<150;jj++) //Copio l'intera sequenza di 150 byte dal buffer al pack-in
				{
					...
					printf ("pack_in_TNC[%d]=%x\t",jj,pack_in_TNC[jj]);
				}	
				printf("\n\n");


				f_TNC=fopen("log_TNC.txt","a");
				//printf("error in the opening log_HL.txt\n");
				fprintf(f_TNC,"%s\n",pack_in_TNC);
				fclose(f_TNC);
			...
			ii=500; //Mi porto al termine del flusso del buffer
		//if(f_TNC=fopen("log_TNC.txt","a")<0)
		//	printf("error in the opening log_TNC.txt\n");
		//write(f_TNC,pack_in_TNC,150);
		//fclose(f_TNC);
		}
		...
		//----TRASFORMAZIONE INTERI IN FLOAT----
		//Conversione in valori corretti
		volt5_curr_f = (float)(5.074*(volt5_curr - FSbasso)/FSalto);
		//printf("\n %d %f %f %f\n",volt5_curr_f,volt5_curr,FSbasso,FSalto); 
		volt3_curr_f = (float)(5.074*(volt3_curr - FSbasso)/FSalto);
		avio_bat1_volt_f = (float)(10.2*(avio_bat1_volt - FSbasso)/FSalto); 
		avio_bat2_volt_f = (float)(10.2*(avio_bat2_volt - FSbasso)/FSalto); 
		avio_bat1_curr_f = (float)(5.34*(avio_bat1_curr - FSbasso)/FSalto);
		avio_bat2_curr_f = (float)(5.34*(avio_bat2_curr - FSbasso)/FSalto); 
		avio_bat1_temp_f = (float)(5.074*(avio_bat1_temp - FSbasso)/FSalto); 
		avio_bat2_temp_f = (float)(5.074*(avio_bat2_temp - FSbasso)/FSalto); 
		avio_bat1_chcurr_f = (float)(6.775*(avio_bat1_chcurr - FSbasso)/FSalto); 
		avio_bat2_chcurr_f = (float)(6.775*(avio_bat2_chcurr - FSbasso)/FSalto); 
		ps_bat1_volt_f = (float)(21.5*(ps_bat1_volt - FSbassoM)/FSaltoM);  
		ps_bat2_volt_f = (float)(21.5*(ps_bat2_volt - FSbassoM)/FSaltoM); 
		ps_bat1_curr_f = (float)(5.34*(ps_bat1_curr - FSbassoM)/FSaltoM);
		ps_bat2_curr_f = (float)(5.34*(ps_bat2_curr - FSbassoM)/FSaltoM);
		ps_bat1_temp_f = (float)(5.074*(ps_bat1_temp - FSbassoM)/FSaltoM); 
		ps_bat2_temp_f = (float)(5.074*(ps_bat2_temp - FSbassoM)/FSaltoM); 
		ps_bat1_chcurr_f = (float)(6.775*(ps_bat1_chcurr - FSbassoM)/FSaltoM); 
		ps_bat2_chcurr_f = (float)(6.775*(ps_bat2_chcurr - FSbassoM)/FSaltoM);  
		step_up_curr_f = (float)(5.34*(step_up_curr - FSbassoM)/FSaltoM);
		step_up_volt_f = (float)(33*(step_up_volt - FSbassoM)/FSalto);

		tmp_temp1 = (float)(5.074*(extra1_temp - FSbasso)/FSalto);
		tmp_temp2 = (float)(5.074*(extra2_temp - FSbasso)/FSalto);
		tmp_temp3 = (float)(5.074*(extra3_temp - FSbasso)/FSalto);
		tmp_temp4 = (float)(5.074*(extra4_temp - FSbasso)/FSalto);
		tmp_temp5 = (float)(5.074*(extra5_temp - FSbasso)/FSalto);
		tmp_temp6 = (float)(5.074*(extra6_temp - FSbasso)/FSalto);
		tmp_temp7 = (float)(5.074*(extra7_temp - FSbasso)/FSalto);

		extra1_temp_f = (float)(0.54643*(tmp_temp1*tmp_temp1*tmp_temp1*tmp_temp1*tmp_temp1)-6.43508*(tmp_temp1*tmp_temp1*tmp_temp1*tmp_temp1)+30.0174*(tmp_temp1*tmp_temp1*tmp_temp1)-68.3413*(tmp_temp1*tmp_temp1)+96.6618*(tmp_temp1)-33.1554);
		extra2_temp_f = (float)(0.54643*(tmp_temp2*tmp_temp2*tmp_temp2*tmp_temp2*tmp_temp2)-6.43508*(tmp_temp2*tmp_temp2*tmp_temp2*tmp_temp2)+30.0174*(tmp_temp2*tmp_temp2*tmp_temp2)-68.3413*(tmp_temp2*tmp_temp2)+96.6618*(tmp_temp2)-33.1554);
		extra3_temp_f = (float)(0.54643*(tmp_temp3*tmp_temp3*tmp_temp3*tmp_temp3*tmp_temp3)-6.43508*(tmp_temp3*tmp_temp3*tmp_temp3*tmp_temp3)+30.0174*(tmp_temp3*tmp_temp3*tmp_temp3)-68.3413*(tmp_temp3*tmp_temp3)+96.6618*(tmp_temp3)-33.1554);
		extra4_temp_f = (float)(0.54643*(tmp_temp4*tmp_temp4*tmp_temp4*tmp_temp4*tmp_temp4)-6.43508*(tmp_temp4*tmp_temp4*tmp_temp4*tmp_temp4)+30.0174*(tmp_temp4*tmp_temp4*tmp_temp4)-68.3413*(tmp_temp4*tmp_temp4)+96.6618*(tmp_temp4)-33.1554);

		//DA CANCELLARE
		//Definizione delle stringhe da stampare nelle entries
		// sprintf(tel_pack_number_c,"%I32d",tel_pack_number); 
		// sprintf(saved_pack_number_c,"%I32d",saved_pack_number); 
	 //    sprintf(year_char,"%d",year); //new add, al posto di 10 c'era year
		// gtk_label_set_text(GTK_LABEL(telemetry35),year_char);
		// sprintf(month_char,"%d",month); //changed
		...
		switch(opmode){
			case 48:
				sprintf(opmode_c,"NOMINAL");
			break;
			case 49:
				sprintf(opmode_c,"RESTART");
			break;
			case 50:
				sprintf(opmode_c,"POWEROFF");
			break;
			case 51:
				sprintf(opmode_c,"RECOVERY");
			break;
			case 52:
				sprintf(opmode_c,"PROPON");
			break;
			case 53:
				sprintf(opmode_c,"BURSTON");
			break;
			default:
				sprintf(opmode_c,"ERROR");
			break;
			
		}
		//sprintf-> print dei valori nelle stringhe
		sprintf(tel_pack_number_c,"%I32d",tel_pack_number); 
		sprintf(saved_pack_number_c,"%I32d",saved_pack_number);
		//sprintf(opmode_c,"%d",opmode);
		sprintf(last_comm_c,"%d",last_comm);
		sprintf(reboot_num_c,"%d",reboot_num);
		...
		sprintf(sec_char,"%d",sec); 
		sprintf(dac_command_c,"%d",dac_command);	
		sprintf(volt5_curr_c,"%.2f",volt5_curr_f);
		sprintf(volt3_curr_c,"%.2f",volt3_curr_f);
		sprintf(avio_bat1_volt_c,"%.2f",avio_bat1_volt_f);//cerca se _volt_c è cosi o è _b
		sprintf(avio_bat2_volt_c,"%.2f",avio_bat2_volt_f);
		sprintf(avio_bat1_curr_c,"%.2f",avio_bat1_curr_f);
		sprintf(avio_bat2_curr_c,"%.2f",avio_bat2_curr_f);
		sprintf(avio_bat1_temp_c,"%.2f",avio_bat1_temp_f);
		sprintf(avio_bat2_temp_c,"%.2f",avio_bat1_temp_f);
		sprintf(avio_bat1_chcurr_c,"%.2f",avio_bat1_chcurr_f);
		sprintf(avio_bat2_chcurr_c,"%.2f",avio_bat2_chcurr_f);
		sprintf(ps_bat1_volt_c,"%.2f",ps_bat1_volt_f);
		sprintf(ps_bat2_volt_c,"%.2f",ps_bat2_volt_f);
		sprintf(ps_bat1_curr_c,"%.2f",ps_bat1_curr_f);
		sprintf(ps_bat2_curr_c,"%.2f",ps_bat2_curr_f);
		sprintf(ps_bat1_temp_c,"%.2f",ps_bat1_temp_f);
		sprintf(ps_bat2_temp_c,"%.2f",ps_bat2_temp_f);
		sprintf(ps_bat1_chcurr_c,"%.2f",ps_bat1_chcurr_f);
		sprintf(ps_bat2_chcurr_c,"%.2f",ps_bat2_chcurr_f);
		sprintf(step_up_curr_c,"%.2f",step_up_curr_f);
		sprintf(step_up_volt_c,"%.2f",step_up_volt_f);
		sprintf(extra1_temp_c,"%.2f",extra1_temp_f);
		sprintf(extra2_temp_c,"%.2f",extra2_temp_f);
		sprintf(extra3_temp_c,"%.2f",extra3_temp_f);
		sprintf(extra4_temp_c,"%.2f",extra4_temp_f);
		sprintf(extra5_temp_c,"%.2f",extra5_temp_f);
		sprintf(extra6_temp_c,"%.2f",extra6_temp_f);
		sprintf(extra7_temp_c,"%.2f",extra7_temp_f);
		sprintf(magnet_x_c,"%.2f",magnet_x_f);
		sprintf(magnet_y_c,"%.2f",magnet_y_f);
		sprintf(magnet_z_c,"%.2f",magnet_z_f);

		//Aggiornamento labels
		gtk_label_set_text(GTK_LABEL(telemetry4),avio_bat1_volt_c);
		gtk_label_set_text(GTK_LABEL(telemetry5),avio_bat1_curr_c);
		gtk_label_set_text(GTK_LABEL(telemetry6),avio_bat1_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry7),avio_bat1_chcurr_c);
		gtk_label_set_text(GTK_LABEL(telemetry8),avio_bat2_volt_c);
		gtk_label_set_text(GTK_LABEL(telemetry9),avio_bat2_curr_c);
		gtk_label_set_text(GTK_LABEL(telemetry10),avio_bat2_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry11),avio_bat2_chcurr_c);
		gtk_label_set_text(GTK_LABEL(telemetry12),ps_bat1_volt_c);
		gtk_label_set_text(GTK_LABEL(telemetry13),ps_bat1_curr_c);
		gtk_label_set_text(GTK_LABEL(telemetry14),ps_bat1_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry15),ps_bat1_chcurr_c);
		gtk_label_set_text(GTK_LABEL(telemetry16),ps_bat2_volt_c);
		gtk_label_set_text(GTK_LABEL(telemetry17),ps_bat2_curr_c);
		gtk_label_set_text(GTK_LABEL(telemetry18),ps_bat2_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry19),ps_bat2_chcurr_c);
		gtk_label_set_text(GTK_LABEL(telemetry20),volt5_curr_c);
		gtk_label_set_text(GTK_LABEL(telemetry21),volt3_curr_c);
		gtk_label_set_text(GTK_LABEL(telemetry22),step_up_volt_c);
		gtk_label_set_text(GTK_LABEL(telemetry23),step_up_curr_c);
		gtk_label_set_text(GTK_LABEL(telemetry28),extra1_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry29),extra2_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry30),extra3_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry31),extra4_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry32),extra5_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry33),extra6_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry34),extra7_temp_c);
		gtk_label_set_text(GTK_LABEL(telemetry35),year_char);
		gtk_label_set_text(GTK_LABEL(telemetry36),month_char);
		gtk_label_set_text(GTK_LABEL(telemetry37),day_char);
		gtk_label_set_text(GTK_LABEL(telemetry38),hour_char);
		gtk_label_set_text(GTK_LABEL(telemetry39),min_char);
		gtk_label_set_text(GTK_LABEL(telemetry40),sec_char);
		gtk_label_set_text(GTK_LABEL(telemetry41),opmode_c);
		gtk_label_set_text(GTK_LABEL(telemetry42),last_comm_c);
		gtk_label_set_text(GTK_LABEL(telemetry43),saved_pack_number_c);
		gtk_label_set_text(GTK_LABEL(telemetry44),reboot_num_c);
		gtk_label_set_text(GTK_LABEL(telemetry45),tel_pack_number_c);
...
//printf("		%f/%f/%f	%f:%f:%f\n	",year,mese_def,giorno_def,ore_def,min_def,sec_def);
		...
		printf("		5V bus current: %f				3V bus current: %fn", volt5_curr_f,volt3_curr_f);
		printf("AV voltage 1: %f	 AV voltage 2: %f	AV current 1: %f	AV current 2: %f\n",avio_bat1_volt_f,avio_bat2_volt_f, avio_bat1_curr_f, avio_bat2_curr_f);
		printf("AV temperature 1: %f	 AV temperature 2: %f	AV current_out 1: %f	AV current_out 2: %f\n",avio_bat1_temp_f,avio_bat2_temp_f, avio_bat1_chcurr_f, avio_bat2_chcurr_f);
		printf("PS voltage 1: %f	 PS voltage 2: %f	PS current 1: %f	PS current 2: %f\n",ps_bat1_volt_f,ps_bat2_volt_f, ps_bat1_curr_f, ps_bat2_curr_f);
		printf("PS temperature 1: %f	 PS temperature 2: %f	PS current_out 1: %f	PS current_out 2: %f\n",ps_bat1_temp_f,ps_bat2_temp_f, ps_bat1_chcurr_f, ps_bat2_chcurr_f);
		printf("Temperaure 1: %f	Temperaure 2: %f	Temperaure 3: %f	Temperaure 4: %f\n",extra1_temp_f, extra2_temp_f, extra3_temp_f, extra4_temp_f);
//printf("Magnetic Field: X:%f	Y:%f	Z:%f\n",magnet_x_f,magnet_y_f,magnet_z_f);
